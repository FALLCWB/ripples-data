...
- **Exclusion criterion (pre-registered):** a repetition is excluded if the action does not complete within the aftermath window (for the N-container actions, if a launch exceeds the 30 s bound). Under `GOGC=off` the per-action accounting is: readback 16 attempted, 12 kept, 4 excluded; 1 container 16/10/6; 10 containers 17/7/10; 50 containers 17/7/10. Under default GC the arm is complete: 13/13, 13/13 and 14/14. Counts are regenerated by `scripts/exclusion_accounting.py` into `data/processed/exclusion_accounting.json`.
